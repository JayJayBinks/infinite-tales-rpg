...
test.describe('Party Lifecycle', () => {
	...
	test('Manual character add/edit/save', async ({ page }) => {
		...
		// Add a new character
		const addCharacterBtn = page
			.getByRole('button', { name: /add.*character|new character/i })
			.first();
	...
	test('Party size guardrails (max 4 characters)', async ({ page }) => {
		...
		const partyCards = page
			.locator('[class*="party"], [class*="character"]')
	...
	test('Start Tale gating (requires non-empty party)', async ({ page }) => {
		...
		if (await startTaleBtn.isVisible().catch(() => false)) {
			...
			const hasParty = await partyCards
				.count()
				.then((c) => c > 0)
				.catch(() => false);

			if (!hasParty) {
				await expect(startTaleBtn)
					.toBeDisabled()
					.catch(() => {
