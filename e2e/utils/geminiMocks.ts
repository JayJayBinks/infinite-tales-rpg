...
function detectAgentType(systemInstruction: string, userMessage: string): AgentType {
  ...
  if (sys.includes('combat agent') || sys.includes('stats update agent')) return 'combatAgent';
  if (sys.includes('event agent') || msg.includes('evaluate the events')) return 'eventAgent';
  ...
  if (sys.includes('game loop agent') || sys.includes('game master agent') || msg.includes('begin the story')) return 'gameAgent';
