	...
	test('Campaign AI generation', async ({ page }) => {
		...
		if (await campaignBtn.isVisible().catch(() => false)) {
			...
			const generateBtn = page
				.getByRole('button', { name: /generate.*campaign|create.*campaign/i })
				.first();
			...
			if (await generateBtn.isVisible().catch(() => false)) {
				...
				const hasChapters = await chaptersList
					.count()
					.then((c) => c > 0)
					.catch(() => false);
	...
	test('Single chapter regeneration', async ({ page }) => {
		...
		if (chapterCount > 1) {
			...
			if (await regenBtn.isVisible().catch(() => false)) {
				...
				const chapterBefore = await page
					.locator('[class*="chapter"]')
					.nth(1)
					.textContent()
					.catch(() => '');
