...
test.describe('UI/UX & Accessibility', () => {
  ...
  test('Screen reader semantics - ARIA landmarks', async ({ page }) => {
    ...
    const hasNav = await navigation.count().then(c => c > 0).catch(() => false);
    if (hasNav) {
  ...
  test('Loading and error states', async ({ page }) => {
    ...
    if (await quickstartButton.isVisible().catch(() => false)) {
      ...
      const loader = page.locator('[class*="loading"], [class*="spinner"], .modal:has-text("Generating")');
  ...
  test('Game settings configuration', async ({ page }) => {
    ...
    const narrationSetting = page.locator('select, input[type="range"], input[type="number"]').first();
