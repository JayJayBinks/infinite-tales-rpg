...
test.describe('Persistence & Recovery', () => {
	...
	test('Session resume validation', async ({ page }) => {
		...
		const hasStory = await newPage
			.locator('h2:has-text("Story")')
			.isVisible()
			.catch(() => false);
		...
		if (hasStory) {
			...
			const isSettings = await newPage
				.locator('text=/settings|api.*key/i')
				.isVisible()
				.catch(() => false);
			if (isSettings) {
	...
	test('Clear data functionality', async ({ page }) => {
		...
		if (await clearBtn.isVisible().catch(() => false)) {
			...
			if (await confirmBtn.isVisible().catch(() => false)) {
				...
				const isCleared = await page
					.locator('text=/cleared|reset|deleted/i')
					.isVisible()
					.catch(() => false);
				if (isCleared) {
