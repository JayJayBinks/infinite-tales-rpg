...
const jsonPrompt = `{
	...
	"party_description": String; Generate a concise party concept: list each member as bullet points with name, archetype/role, one vivid trait, and a distinguishing hook; 1-4 members only; avoid clichés and overused tropes unless subverted.,
	"party_count": String; Provide only the integer number of party members you generated (1-4 range). Return just the number, no words,
	...
	async generateCampaign(
		...
		if (characterDescription?.name) {
			request.historyMessages?.push({
				role: 'user',
				content:
					'The campaign story must have the party_description as player character protagonist: ' +
