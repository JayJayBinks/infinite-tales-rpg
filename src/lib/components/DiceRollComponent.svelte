<script lang="ts">
    ...
    let diceRollRequiredValueState = $derived(diceRollLogic.getRequiredValue(action?.action_difficulty, difficultyState.value));
    let modifierReasonState = $derived(action?.dice_roll?.modifier_explanation);
    let modifierState = $derived(Number.parseInt(action?.dice_roll?.modifier_value as unknown as string) || 0);
