		...
		<div class="mt-4">
			<PartyMemberSwitcher
				...
				onSwitch={async () => {
					...
					if (characterActionsByMemberState.value[activeId]) {
						characterActionsState.value = characterActionsByMemberState.value[activeId];
					} else {
						characterActionsState.reset();
						// Actions will be generated by the normal flow
					}
					
					// Render the game state with new character
					if (currentGameActionState && characterActionsState.value.length > 0) {
						renderGameState(currentGameActionState, characterActionsState.value);
					}
				}}
			/>
